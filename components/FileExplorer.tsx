...
import { HtmlIcon, CssIcon, JsIcon, DocumentTextIcon, FolderIcon, IconProps, NewFileIcon, NewFolderIcon, FileUploadIcon, PlayIcon, DownloadIcon } from './Icons';
...
const getFileIcon = (path: string): React.ReactElement<IconProps> => {
    ...
    if (path.endsWith('.ts') || path.endsWith('.tsx')) return <JsIcon className="h-5 w-5 text-blue-400" />;
    if (path.endsWith('.md')) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-end gap-1 p-1 border-b border-[var(--card-border)] mb-2">
         ...
         <input type="file" ref={uploadInputRef} onChange={handleFileChange} className="hidden" multiple accept=".html,.css,.js,.jsx,.ts,.tsx,.md,.zip" />
      </div>
